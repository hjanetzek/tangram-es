...
struct TileID {
    ...
    void getHierarchyTiles(std::vector<TileID>& _hierarchyTiles, int maxZoomLvl) {
        if( (x >> 1) >= 0 && (y >> 1) >= 0 && (z-1) >= 0) {
            _hierarchyTiles.push_back(TileID(x >> 1, y >> 1, z-1));
        }
        if((z + 1) <= maxZoomLvl) {
            for(int i = 0; i < 2; i++) {
                int xVal = (x << 1) + i;
                for(int j = 0; j < 2; j++) {
                    int yVal = (y << 1) + j;
                    _hierarchyTiles.push_back(TileID(xVal, yVal, z+1));
                }
            }
        }
    }
    ...
    void getChildren(std::vector<TileID>& _children, int maxZoomLvl) {
        if((z + 1) <= maxZoomLvl) {
            for(int i = 0; i < 2; i++) {
                int xVal = (x << 1) + i;
                for(int j = 0; j < 2; j++) {
